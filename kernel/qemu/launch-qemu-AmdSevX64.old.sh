...
usage() {
	...
	echo " -oldfw		          boot with simple OVMF"
...
EXEC_PATH=""
if [ -n "${OLDFW}" ]; then
	# EXEC_PATH="./qemu_old/usr/local"
	EXEC_PATH="./qemu_new/usr/local"
else
	EXEC_PATH="./qemu_new/usr/local"
fi
...
# add_opts "-enable-kvm -cpu ${CPU_MODEL} -machine pc-q35-7.1"
...
if [ "$GPU" = "1" ]; then
	...
	add_opts "-device vfio-pci,host=41:00.0,bus=pci.1"
...
if [ -n "${HDA}" ]; then
	if [ "$USE_VIRTIO" = "1" ]; then
		if [[ ${HDA} = *"qcow2" ]]; then
			if [ -n "${READONLY_HDA}" ]; then
				add_opts "-drive file=${HDA},if=none,id=disk0,format=qcow2,readonly"
			else
...
if [ ${SEV} = "1" ]; then
	...
	if [ "${SEV_SNP}" = 1 ]; then
		...
		if [ -n "${OLDFW}" ]; then
			...
			add_opts "-object sev-snp-guest,id=sev0,cbitpos=${CBITPOS},reduced-phys-bits=1,discard=${DISCARD},kernel-hashes=on"
		fi
